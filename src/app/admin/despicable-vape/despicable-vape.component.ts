...
@Component({
  selector: "app-despicable-vape",
  templateUrl: "./despicable-vape.component.html",
  styleUrls: ["./despicable-vape.component.css"],
})
export class DespicableVapeComponent implements OnInit {
  imgSrc: string;
  imgSrc2: string;
  imgSrc3: string;
  ...
  despicableVapeForm = new FormGroup({
  ...
  ngOnInit(): void {
    //fetching the data
    this.service.despicableVapeDetails.snapshotChanges().subscribe((list) => {
  ...
  //   if (this.despicableVapeForm.valid) {
  ...
  //             console.log(this.despicableVapeForm.value);
  //             console.log(formValue);
  //             this.service.insertDespicableVapeDetails(
  //               this.despicableVapeForm.value
  //             );
  //             console.log(formValue);
  //             this.resetForm();
  //           });
                        ...
                        .pipe(
                          finalize(() => {
                            fileRef3.getDownloadURL().subscribe((url3) => {
                              ...
                              console.log(formValue);
                              this.service.insertDespicableVapeDetails(this.despicableVapeForm.value);
                              console.log(formValue);
                              this.resetForm();
                            });
                          })
  ...
  //
  get formControls() {
    return this.despicableVapeForm["controls"];
  }

  //resetting the form after submit
  resetForm() {
    this.despicableVapeForm.reset();
    this.despicableVapeForm.setValue({
  ...
  //deleting
  deleteVapeItem(item, itemID) {
    console.log("Deleting item with key:", itemID);

    const cigarettesListRef = firebase.database().ref("despicableVapeList");
    let itemKey;
